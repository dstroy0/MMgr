 ...
 */
#define MMGR_SHA256_ROTATE(value_, places_) (((value_) >> (places_)) | ((value_) << (32u - (places_))))
...
static void mmgr_sha256_compress(uint32_t *state, const uint8_t *block)
{
    ...
    for (unsigned round = 0u; round < 64u; round++)
    {
        ...
        const uint32_t second =
            mmgr_sha256_big_sigma0(working[0]) + mmgr_sha256_majority(working[0], working[1], working[2]);
...
static const MmgrSha256Vector s_vectors[4] = {
    ...
    },
    {
        ...
        10u,
        {0x59u, 0x48u, 0x47u, 0x32u, 0x84u, 0x51u, 0xbdu, 0xfau, 0x85u, 0x05u, 0x62u,
         0x25u, 0x46u, 0x2cu, 0xc1u, 0xd8u, 0x67u, 0xd8u, 0x77u, 0xfbu, 0x38u, 0x8du,
         0xf0u, 0xceu, 0x35u, 0xf2u, 0x5au, 0xb5u, 0x56u, 0x2bu, 0xfbu, 0xb5u},
    },
};
