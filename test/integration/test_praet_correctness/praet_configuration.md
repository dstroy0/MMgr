...
**Scope:** `test/integration/test_praet_correctness/praet_defaults.h`, `praet_clock.h`, `praet_platform_detection.h`, `praet_config_verdict.h`, `praet_schedule.h`
...
Six build knobs arrive as `-D` on the command line or as a `#define` ahead of the include. Four of them describe the part, and are read in `praet_defaults.h`. Two more describe the clock, and are read in `praet_clock.h`, which also reads a seventh knob on one arm.

One answer is given at the declaration instead, as a token passed to `PraetScheduleContext` (`praet_schedule.h:214-219`). It is not a build knob, and setting the build knob that used to hold it stops the build (`praet_defaults.h:115-117`).

Several facts are derived from the architecture and are not answerable at all. `praet_platform_detection.h` reads what the compiler predefines and settles which family this is, how wide its register is, and whether it defines a cycle counter. One of those decides what an unanswered clock source falls back to (`praet_clock.h:71-80`).

`MMGR_ACCEPT_DEFAULTS` is the last piece. It changes nothing about which value a knob takes. What it changes is whether an incomplete configuration stops the build (`praet_config_verdict.h:31-36`).

## How an unanswered knob reports

Every knob that was not set takes a documented default, raises a warning naming itself and the value it took, and leaves a flag behind. No knob stops the build where it sits. `praet_config_verdict.h` reads the flags after every knob has spoken and stops once (`praet_config_verdict.h:33-36`).
...
Logical channels one context carries. Sizes every per-channel array in `PraetSchedule` (`praet_schedule.h:265-275`).

Default 8, with a warning (`praet_defaults.h:50-54`).

### PRAET_SETTLE_MICROS

Microseconds the engine spends coming up before it will take a transfer. One timer for the context, because settling is a property of the engine and not of a channel (`praet_schedule.h:249-250`).

Default 0, with a warning (`praet_defaults.h:64-68`). A build taking that default waits for nothing after an attach.
...
Default 1000, with a warning (`praet_defaults.h:81-85`).

### PRAET_RECOVERY

Whether a stalled transfer can be backed out or scrubbed. Legal values are 0 and 1, and a third value stops the build where it sits (`praet_defaults.h:108-110`). That one does not join the basket, because every later gate would be reading a value nobody can interpret.

Default 0, with a warning (`praet_defaults.h:98-102`).

With it on, the context carries `bound`, `bound_bytes`, `start`, `length`, `position`, `boundary_crc`
and `word_boundary_crc` (`praet_schedule.h:267-275`), and six entries exist that do not otherwise:
`praet_schedule_submit` taking an offset and a length, `praet_schedule_kick` taking a position,
`praet_schedule_position`, `praet_schedule_touched`, `praet_schedule_boundary_crc`
(`praet_schedule.h:417-519`) and `praet_schedule_resolve` (`praet_schedule.h:605-623`).

With it off, `praet_schedule_submit` takes no span and `praet_schedule_kick` takes no position
(`praet_schedule.h:521-562`). A call site written for the other form fails to compile. The watchdog
still runs, so a channel still reads stalled; what is gone is any statement about which bytes were
touched.

### PRAET_CLOCK_HZ

Ticks the clock counts in one second. Every deadline in this module is microseconds, and this is what a tick is scaled against (`praet_clock.h:127`).

Two conditions hold at compile time. The frequency divides evenly into microseconds, and it is at least one megahertz (`praet_clock.h:142-147`). Every part this library targets runs at a whole number of megahertz, and a clock below one megahertz cannot resolve a microsecond.

Default 1000000, with a warning (`praet_clock.h:55-59`). That default reads a tick as a microsecond. It is not an estimate of any part's frequency, because an estimate would make every deadline wrong by however much it missed by.

### PRAET_CLOCK_SOURCE

`PRAET_CLOCK_CALLER` where the caller already runs a counter this reads. `PRAET_CLOCK_OWN` where this pins its own timer to a core. A third value stops the build (`praet_clock.h:84-86`).

Unset, it is derived from `PRAET_PLATFORM_HAS_CYCLE_COUNTER` (`praet_clock.h:71-80`). Both arms warn, and each says which way it went and why.

### PRAET_CLOCK_CORE

The core a pinned timer runs on. Read only where the source is `PRAET_CLOCK_OWN`, and setting it on the other arm stops the build (`praet_clock.h:115-117`).

Default `PRAET_PLATFORM_CLOCK_CORE`, with a warning (`praet_clock.h:105-109`).
...
```c
PraetScheduleContext(s_schedule, RECOVERY_WORD_BOUNDARY_BITWISE_CRC_DISABLE);
```

The second argument answers whether a recovery checksums the word the engine was inside. It is one of two tokens (`praet_schedule.h:77-81`), and both of them report at every build that declares a context (`praet_schedule.h:116-125`). A choice that only spoke up one way would train a reader to take its silence as the safe answer, and neither answer here is safe by default.

The token is independent of `PRAET_RECOVERY`. Turning recovery on does not turn the check on, and asking for the check on a build with recovery off fails an assertion naming what to do (`praet_schedule.h:216-218`).

Three things are refused. A misspelled token fails on an unknown type name carrying the token it was given, since the declarator pastes it onto a type that exists for the two spellings and nothing else (`praet_schedule.h:133`). A plain `1` or `TRUE` fails the same way, because those are values and neither names a type. `PRAET_RECOVERY_CRC` as a build knob is refused outright (`praet_defaults.h:115-117`).
...
PraetScheduleContext(s_frame_dma, RECOVERY_WORD_BOUNDARY_BITWISE_CRC_ENABLE);
PraetChannel(s_frame_dma, 0, frame_pool);
```

`PraetChannel` says which pool a channel is over (`praet_schedule.h:384-392`). It emits an enumerator
whose name carries the context, the channel and the pool. `PraetAttach` and `PraetSubmit` both name
that enumerator (`praet_schedule.h:403`), so reaching either with a pool the channel is not over is an
...
(`praet_schedule.h:385-388`). The entry still refuses an out of range channel at run time, and the
...
`PraetAttach` hands the entry `mmgr_pars_storage_##pool_` and `pool_##_bytes` (`praet_schedule.h:360-363`).
...
`praet_schedule_submit` takes no pointer (`praet_schedule.h:441`). The address came from the pool named
at the attach, so a transfer adds an offset and a length. `PraetSubmit` compares that span against
`sizeof(mmgr_pars_storage_##pool_)` while compiling (`praet_schedule.h:461-466`). That is what the
...
(`praet_flag_map.h:258-263`), which are token ids the same way the statuses are. A misspelling is an
undeclared identifier carrying the name that was written. The count of them is asserted to fit the
descriptor field (`praet_flag_map.h:325`).

## The poll

`praet_schedule_poll` is what a caller calls, and the only thing they have to (`praet_schedule.h:680`).
...
The port is asked before the short circuit, not after (`praet_schedule.c:376-407`). A part with no
...
`praet_hw_progress` is the fifth port hook (`praet_schedule.h:299`). The four in
...
| Any knob unset, no `MMGR_ACCEPT_DEFAULTS` | one warning per unset knob, then one error | `praet_config_verdict.h:33-36` |
| Any knob unset, `MMGR_ACCEPT_DEFAULTS` defined | one warning per unset knob, builds | `praet_config_verdict.h:31` |
| `PRAET_RECOVERY` neither 0 nor 1 | error | `praet_defaults.h:108-110` |
| `PRAET_RECOVERY_CRC` defined at all | error | `praet_defaults.h:115-117` |
| `PRAET_CLOCK_SOURCE` neither token | error | `praet_clock.h:84-86` |
| `PRAET_CLOCK_SOURCE` is `PRAET_CLOCK_OWN`, architecture has no counter | error | `praet_clock.h:91-93` |
| `PRAET_CLOCK_CORE` set, source is `PRAET_CLOCK_CALLER` | error | `praet_clock.h:115-117` |
| `PRAET_CLOCK_HZ` not a whole number of megahertz | assert | `praet_clock.h:142-143` |
| `PRAET_CLOCK_HZ` below one megahertz | assert | `praet_clock.h:145-147` |
...
| Declaration token misspelled | error naming the token given | `praet_schedule.h:133` |
| Declaration token is a value, such as `1` | error naming the value given | `praet_schedule.h:143` |
| `RECOVERY_WORD_BOUNDARY_BITWISE_CRC_ENABLE` with `PRAET_RECOVERY` 0 | assert | `praet_schedule.h:216-218` |
| A channel submitting a span of a pool it is not over | error naming the context, channel and pool | `praet_schedule.h:403` |
| A channel attached over a pool it is not declared over | error naming the context, channel and pool | `praet_schedule.h:403` |
| A channel the context never declared with `PraetChannel` | error naming the context, channel and pool | `praet_schedule.h:403` |
| A span past the pool, by length or by offset | error naming the span | `praet_schedule.h:461-466` |
| An ordinary array where a pool belongs | error naming the array | `praet_schedule.h:360-363` |
| `PraetChannel` on a channel past `PRAET_CHANNELS` | assert | `praet_schedule.h:385-386` |
| `PraetChannel` over a pool with no bytes | assert | `praet_schedule.h:387-388` |
| A region that is neither token | error naming the region written | `praet_flag_map.h:258-263` |
| More region token ids than the descriptor field holds | assert | `praet_flag_map.h:325` |
| Two statuses sharing a token id, or one missing from the list | assert | `praet_flag_map.h:290` |
| Region descriptor overlapping a status | assert | `praet_flag_map.h:295` |
| Region descriptor off a byte boundary | assert | `praet_flag_map.h:304` |
| Region descriptor past the top of the flag word | assert | `praet_flag_map.h:306` |
| The field write unable to fill the region, or overrunning it | assert | `praet_flag_map.h:312-315` |
| Two core states sharing a value, or one past the mask | assert | `praet_flag_map.h:283` |
| An entry clearing a bit inside the region | assert | `praet_schedule.c:62` |
| An entry clearing a bit the map does not account for | assert | `praet_schedule.c:65` |
...
same two lines produce the `praet_clock.h:91-93` error alongside the token's line. The S3 build has
