...
**Scope:** `src/memoriam_praetereo/praet_praefinitum.h`, `praet_horologiorum_custos.h`, `praet_platform_detection.h`, `praet_iudex.h`, `praet_ordo.h`, `praet_ordo.c`

The schedule is in `src/memoriam_praetereo/` and compiles when `MMGR_ENABLE_DMA` is on. Every path below without a directory is in that one.
...
(`praet_tabula_vexillorum.h:257-261`), which are token ids the same way the statuses are. A mistyped token is an
undeclared identifier carrying the name that was written. The count of them is asserted to fit the
descriptor field (`praet_tabula_vexillorum.h:324`).
...
The port is asked before the short circuit, not after (`praet_ordo.c:470-492`). A part with no
...
It carries a weak refusing default that answers zero (`praet_ordo.c:417`), which
`praet_ordo_take_progress` reads as no movement. A running channel nothing reports on is then marked
stalled once its keepalive window passes, and `test_memoriam_praetereo` holds an unported build to
that.

The default sits in `praet_ordo.c` because that is the unit that calls it, the way the other four sit
beside their calls in `memoriam_praetereo.c`. GCC on PE-COFF emits a weak definition as a weak
external. The body lands under `.weak.praet_hw_progress.` and the name itself stays undefined in the
object, and `ld` will not resolve it from another translation unit. Inside its own unit it does
resolve, which is the arrangement every linker handles. None of this has run on an ELF target, which
is where every part this module is for lives.

`test_praet_correctness.c` includes `praet_engine.c`, which defines the hook strongly in the suite's
own unit. The library object carries the weak default, and the strong definition wins at the link.
...
Unaligned access is `EMBED_FAST_UNALIGNED_LOAD`, which `embed_compiler_directives.h:375-382` already provides. This module does not define its own.
