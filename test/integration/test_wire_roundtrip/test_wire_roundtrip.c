...
void test_a_byte_written_is_the_byte_read(void)
{
    ...
    MMGR_CALL(byteio.put_be, OctetusCfg, .w = &w, .val = (uint64_t)0x5Au, .bytes = (size_t)1);
...
void test_big_endian_fields_round_trip_at_every_width(void)
{
    ...
    for (unsigned i = 0; i < sizeof cases / sizeof cases[0]; i++)
    {
        ...
        TEST_ASSERT_TRUE(MMGR_CALL(byteio.take_be, OctetusCfg, .r = &r, .out = &got, .bytes = cases[i].n));
