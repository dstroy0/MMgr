...
static double muto_round_probe(MutoCtx *c, embed_bool neg)
{
    c->neg = neg;
    return muto_round(c);
}
...
{
    static const embed_u64 vals[] = {0x8000000000000000ULL, 0xFFFFFFFFFFFFFFFFULL, 0x9E3779B97F4A7C15ULL,
    ...
    for (unsigned i = 0; i < sizeof vals / sizeof vals[0]; i++)
    {
        for (unsigned j = 0; j < sizeof vals / sizeof vals[0]; j++)
        {
            ...
            const embed_u64 m = 0xFFFFFFFFULL;
            const embed_u64 a0 = vals[i] & m;
            const embed_u64 a1 = vals[i] >> 32;
            const embed_u64 b0 = vals[j] & m;
            const embed_u64 b1 = vals[j] >> 32;
            const embed_u64 t0 = a0 * b0;
            const embed_u64 t1 = a1 * b0 + (t0 >> 32);
            const embed_u64 t2 = a0 * b1 + (t1 & m);
            const embed_u64 want_lo = (t2 << 32) | (t0 & m);
            const embed_u64 want_hi = a1 * b1 + (t1 >> 32) + (t2 >> 32);
...
static double round_of(embed_u64 mant53, unsigned half, unsigned rest, int e2)
{
    MutoCtx f;

    f.hi = (mant53 << 11) | ((embed_u64)half << 10);
    f.lo = 0u;
    f.fe2 = e2;
    f.rest = (int)rest;
    return muto_round_probe(&f, EMBED_FALSE);
}

void test_an_exact_tie_goes_to_even(void)
{
    const embed_u64 even = ((embed_u64)1 << 52) | 0u;
    const embed_u64 odd = ((embed_u64)1 << 52) | 1u;
...
void test_rounding_a_fraction_of_nothing(void)
{
    ...
    TEST_ASSERT_EQUAL_DOUBLE(0.0, muto_round_probe(&f, EMBED_FALSE));
    TEST_ASSERT_TRUE_MESSAGE(
        EMBED_CALL(fract.sign, FractioCfg,
                   .bits = EMBED_CALL(fract.to_bits, FractioCfg, .val = muto_round_probe(&f, EMBED_TRUE))) != 0u,
        "and it keeps a sign it was given");
}

static embed_u64 to_u64_of(embed_u64 hi, embed_u64 lo, embed_iword fe2)
{
    MutoCtx f;
...
{
    TEST_ASSERT_EQUAL_UINT64(0u, to_u64_of(0u, 0u, -100));
}

void test_to_u64_of_a_number_wider_than_the_word_saturates(void)
{
    TEST_ASSERT_EQUAL_UINT64(~(embed_u64)0, to_u64_of(1u, 0u, -32));
}

void test_to_u64_with_the_point_on_the_word_boundary(void)
{
    TEST_ASSERT_EQUAL_UINT64(7u, to_u64_of(7u, 0u, -64));
}

void test_to_u64_on_the_boundary_rounds_a_tie_up(void)
{
    TEST_ASSERT_EQUAL_UINT64_MESSAGE(5u, to_u64_of(4u, (embed_u64)1 << 63, 0 - 64), "an even half goes up");
    TEST_ASSERT_EQUAL_UINT64_MESSAGE(6u, to_u64_of(5u, (embed_u64)1 << 63, 0 - 64), "an odd half goes up");
}

void test_to_u64_on_the_boundary_ignores_what_is_under_the_round_bit(void)
{
    TEST_ASSERT_EQUAL_UINT64_MESSAGE(5u, to_u64_of(4u, ((embed_u64)1 << 63) | ((embed_u64)1 << 62), -64),
                                     "bits under the half do not add to one that is already going up");
    TEST_ASSERT_EQUAL_UINT64_MESSAGE(4u, to_u64_of(4u, (embed_u64)1 << 62, -64),
                                     "and they do not lift a value with no half bit at all");
}
