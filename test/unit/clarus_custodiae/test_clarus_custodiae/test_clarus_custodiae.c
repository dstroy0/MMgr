...
void test_a_pool_that_has_not_bound_yet_answers_for_nothing(void)
{
    ...
    TEST_ASSERT_EQUAL_INT_MESSAGE(-1, clarus.slot_of(elsewhere), "an unbound pool has no slot to name");
...
void test_slot_of_names_the_tenant(void)
{
    base_mark = clarus.mark();
    const void *p = clarus.alloc(8u, 1u);
    const int slot = clarus.slot_of(p);

    TEST_ASSERT_GREATER_OR_EQUAL_INT(0, slot);
    TEST_ASSERT_LESS_THAN_INT(MMGR_REG_POOL_SLOTS, slot);
}
...
void test_slot_of_something_outside_the_pool_is_minus_one(void)
{
    base_mark = clarus.mark();
    uint8_t elsewhere[8];
    TEST_ASSERT_EQUAL_INT(-1, clarus.slot_of(elsewhere));
    TEST_ASSERT_EQUAL_INT(-1, clarus.slot_of(NULL));
}
