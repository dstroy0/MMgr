...
{
    return iteratio_infinita.open(&(InfinCfg){.ring = &ring, .owner = &owner});
}
...
{
    TEST_ASSERT_EQUAL_size_t(0u, iteratio_infinita.available(&(InfinCfg){.ring = &ring}));
    TEST_ASSERT_EQUAL_size_t_MESSAGE(CAP - 1u, iteratio_infinita.vacant(&(InfinCfg){.ring = &ring}),
...
void test_write_moves_what_available_reports(void)
{
    ...
    TEST_ASSERT_NOT_NULL(iteratio_infinita.singularitas(&(InfinCfg){.ring = &ring, .src = src, .bytes = 8u, .sing = &bytewise}));
    TEST_ASSERT_EQUAL_size_t(8u, iteratio_infinita.available(&(InfinCfg){.ring = &ring}));
    TEST_ASSERT_EQUAL_size_t(CAP - 1u - 8u, iteratio_infinita.vacant(&(InfinCfg){.ring = &ring}));
}
...
void test_a_raw_read_names_the_bytes_and_consumes_nothing(void)
{
    ...
    const uint8_t *const at = iteratio_infinita.read(&(InfinCfg){.ring = &ring, .cur = cur, .bytes = 4u});
...
void test_a_raw_read_of_more_than_is_there_is_null(void)
{
    ...
    iteratio_infinita.singularitas(&(InfinCfg){.ring = &ring, .src = src, .bytes = 4u, .sing = &bytewise});
    TEST_ASSERT_NULL(iteratio_infinita.read(&(InfinCfg){.ring = &ring, .cur = cur, .bytes = 5u}));
}

void test_a_raw_read_of_an_empty_ring_is_null(void)
{
    struct MmgrCursor *const cur = cursor();
    TEST_ASSERT_NULL(iteratio_infinita.read(&(InfinCfg){.ring = &ring, .cur = cur, .bytes = 1u}));
}
...
void test_consume_is_what_moves_the_tail(void)
{
    ...
    iteratio_infinita.singularitas(&(InfinCfg){.ring = &ring, .src = src, .bytes = 8u, .sing = &bytewise});
    iteratio_infinita.consume(&(InfinCfg){.ring = &ring, .cur = cur, .bytes = 3u});
    TEST_ASSERT_EQUAL_size_t(5u, iteratio_infinita.available(&(InfinCfg){.ring = &ring}));
}
...
void test_read_byte_takes_one_and_refuses_an_empty_ring(void)
{
    ...
    TEST_ASSERT_FALSE_MESSAGE(
        iteratio_infinita.read_byte(&(InfinCfg){.ring = &ring, .cur = cur, .dst = &got}),
        "an empty ring has no byte to hand back");

    iteratio_infinita.singularitas(&(InfinCfg){.ring = &ring, .src = src, .bytes = 2u, .sing = &bytewise});
    TEST_ASSERT_TRUE(iteratio_infinita.read_byte(&(InfinCfg){.ring = &ring, .cur = cur, .dst = &got}));
    TEST_ASSERT_EQUAL_HEX8(0x5Au, got);
    TEST_ASSERT_EQUAL_size_t(1u, iteratio_infinita.available(&(InfinCfg){.ring = &ring}));
}
...
void test_peek_copies_without_consuming(void)
{
    ...
    iteratio_infinita.singularitas(&(InfinCfg){.ring = &ring, .src = src, .bytes = 4u, .sing = &bytewise});
    iteratio_infinita.peek(&(InfinCfg){.ring = &ring, .cur = cur, .dst = dst, .bytes = 2u, .off = 1u});

    TEST_ASSERT_EQUAL_HEX8(0x22u, dst[0]);
    TEST_ASSERT_EQUAL_HEX8(0x33u, dst[1]);
    TEST_ASSERT_EQUAL_size_t(4u, iteratio_infinita.available(&(InfinCfg){.ring = &ring}));
}
...
void test_a_write_that_wraps_comes_back_in_order(void)
{
    ...
    {
        iteratio_infinita.singularitas(&(InfinCfg){.ring = &ring, .src = src, .bytes = 16u, .sing = &bytewise});
        iteratio_infinita.consume(&(InfinCfg){.ring = &ring, .cur = cur, .bytes = 16u});
    }
    static const uint8_t run[8] = {1u, 2u, 3u, 4u, 5u, 6u, 7u, 8u};
    TEST_ASSERT_NOT_NULL(iteratio_infinita.singularitas(&(InfinCfg){.ring = &ring, .src = run, .bytes = 8u, .sing = &bytewise}));
    TEST_ASSERT_EQUAL_size_t(8u, iteratio_infinita.available(&(InfinCfg){.ring = &ring}));

    uint8_t got[8] = {0};
    iteratio_infinita.peek(&(InfinCfg){.ring = &ring, .cur = cur, .dst = got, .bytes = 8u, .off = 0u});
...
void test_seek_moves_a_cursor_inside_its_frame(void)
{
    ...
    iteratio_infinita.seek(&(InfinCfg){.ring = &ring, .cur = cur, .off = 32u});
    TEST_ASSERT_EQUAL_size_t_MESSAGE(32u, cur->off, "the cursor is an offset, not a pointer");

    iteratio_infinita.seek(&(InfinCfg){.ring = &ring, .cur = cur, .off = 0u});
...
#else
    size_t tess = 0u;
    TEST_ASSERT_NULL_MESSAGE(iteratio_infinita.drain(&(InfinCfg){.ring = &ring, .from = 0u,
