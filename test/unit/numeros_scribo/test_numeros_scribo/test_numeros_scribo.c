...
void test_emit_a_literal(void)
{
    ...
    TEST_ASSERT_EQUAL_size_t(
        5u, EMBED_CALL(numer.emit, NumerosCfg, .out = b, .cap = sizeof b, .vals = fields, .nvals = 1u));
...
void test_emit_mixes_kinds(void)
{
    ...
    EMBED_CALL(numer.emit, NumerosCfg, .out = b, .cap = sizeof b, .vals = fields, .nvals = 4u);
...
void test_emit_carries_width(void)
{
    ...
    EMBED_CALL(numer.emit, NumerosCfg, .out = b, .cap = sizeof b, .vals = fields, .nvals = 2u);
...
void test_emit_default_width_when_unstated(void)
{
    ...
    EMBED_CALL(numer.emit, NumerosCfg, .out = b, .cap = sizeof b, .vals = fields, .nvals = 1u);
...
void test_emit_takes_as_many_values_as_it_is_given(void)
{
    ...
    EMBED_CALL(numer.emit, NumerosCfg, .out = b, .cap = sizeof b, .vals = fields,
...
void test_emit_overflow_reports_and_terminates(void)
{
    ...
    TEST_ASSERT_EQUAL_size_t_MESSAGE(
        0u, EMBED_CALL(numer.emit, NumerosCfg, .out = b, .cap = sizeof b, .vals = fields, .nvals = 1u),
        "overflow returns 0");
...
void test_emit_append_builds_on_what_is_there(void)
{
    ...
    EMBED_CALL(numer.emit, NumerosCfg, .out = b, .cap = sizeof b, .vals = head, .nvals = 1u);
    TEST_ASSERT_EQUAL_size_t(
        9u, EMBED_CALL(numer.emit_append, NumerosCfg, .out = b, .cap = sizeof b, .vals = tail, .nvals = 1u));
...
void test_emit_escapes(void)
{
    ...
    EMBED_CALL(numer.emit, NumerosCfg, .out = b, .cap = sizeof b, .vals = json, .nvals = 1u);
    TEST_ASSERT_EQUAL_STRING_MESSAGE("\"a\\\"b\"", b, "json emits a whole JSON string, quotes included");

    EMBED_CALL(numer.emit, NumerosCfg, .out = b, .cap = sizeof b, .vals = xml, .nvals = 1u);
...
void test_emit_rejects_an_unknown_kind(void)
{
    ...
    TEST_ASSERT_EQUAL_size_t(0u,
                             EMBED_CALL(numer.emit, NumerosCfg, .out = b, .cap = sizeof b, .vals = &bad, .nvals = 1u));
...
void test_emit_of_nothing_is_empty_not_garbage(void)
{
    ...
    b[0] = 'x';
    TEST_ASSERT_EQUAL_size_t(
        0u, EMBED_CALL(numer.emit, NumerosCfg, .out = b, .cap = sizeof b, .vals = s_none, .nvals = 0u));
    TEST_ASSERT_EQUAL_STRING("", b);
}
...
void test_build_renders_a_spec(void)
{
    ...
    TEST_ASSERT_EQUAL_size_t(
        5u, EMBED_CALL(numer.build, NumerosCfg, .out = out, .cap = sizeof out, .spec = spec, .vals = v, .nvals = 1u));
...
void test_build_rejects_a_value_of_the_wrong_kind(void)
{
    ...
    TEST_ASSERT_EQUAL_size_t_MESSAGE(
        0u, EMBED_CALL(numer.build, NumerosCfg, .out = out, .cap = sizeof out, .spec = spec, .vals = v, .nvals = 1u),
...
void test_build_rejects_too_few_and_too_many_values(void)
{
    ...
    TEST_ASSERT_EQUAL_size_t(
        0u, EMBED_CALL(numer.build, NumerosCfg, .out = out, .cap = sizeof out, .spec = spec, .vals = one, .nvals = 1u));
    TEST_ASSERT_EQUAL_size_t_MESSAGE(
        0u,
        EMBED_CALL(numer.build, NumerosCfg, .out = out, .cap = sizeof out, .spec = spec, .vals = three, .nvals = 3u),
...
void test_build_guards_its_arguments(void)
{
    ...
    TEST_ASSERT_EQUAL_size_t(
        0u, EMBED_CALL(numer.build, NumerosCfg, .out = out, .cap = (size_t)0, .spec = spec, .vals = v, .nvals = 1u));
    TEST_ASSERT_EQUAL_size_t_MESSAGE(
        0u, EMBED_CALL(numer.build, NumerosCfg, .out = out, .cap = sizeof out, .spec = spec, .vals = v, .nvals = 0u),
...
void test_build_of_a_spec_with_only_literals(void)
{
    ...
    TEST_ASSERT_EQUAL_size_t(6u, EMBED_CALL(numer.build, NumerosCfg, .out = out, .cap = sizeof out, .spec = spec,
                                            .vals = s_none, .nvals = 0u));
...
void test_build_covers_every_kind(void)
{
    ...
    TEST_ASSERT_GREATER_THAN_size_t(
        0u, EMBED_CALL(numer.build, NumerosCfg, .out = out, .cap = sizeof out, .spec = spec, .vals = v, .nvals = 7u));
    TEST_ASSERT_TRUE(EMBED_CALL(cellul.has, CatenaFinitaCfg, .src = out, .cap = sizeof out, .other = "\"j\"",
                                .other_cap = 4u, .ci = EMBED_FALSE));
    TEST_ASSERT_TRUE(EMBED_CALL(cellul.has, CatenaFinitaCfg, .src = out, .cap = sizeof out, .other = "&lt;x&gt;",
                                .other_cap = 10u, .ci = EMBED_FALSE));
}
...
void test_build_covers_the_width_bearing_kinds(void)
{
    ...
    TEST_ASSERT_GREATER_THAN_size_t(
        0u, EMBED_CALL(numer.build, NumerosCfg, .out = out, .cap = sizeof out, .spec = spec, .vals = v, .nvals = 5u));
    TEST_ASSERT_TRUE_MESSAGE(EMBED_CALL(cellul.has, CatenaFinitaCfg, .src = out, .cap = sizeof out, .other = "0007",
                                        .other_cap = 5u, .ci = EMBED_FALSE),
                             "DEC pads to its width");
    TEST_ASSERT_TRUE_MESSAGE(EMBED_CALL(cellul.has, CatenaFinitaCfg, .src = out, .cap = sizeof out, .other = "00ab",
                                        .other_cap = 5u, .ci = EMBED_FALSE),
                             "HEX pads to its width");
    TEST_ASSERT_TRUE_MESSAGE(EMBED_CALL(cellul.has, CatenaFinitaCfg, .src = out, .cap = sizeof out, .other = "10",
...
void test_build_overflow_leaves_an_empty_buffer(void)
{
    ...
    TEST_ASSERT_EQUAL_size_t(0u, EMBED_CALL(numer.build, NumerosCfg, .out = out, .cap = sizeof out, .spec = spec,
                                            .vals = s_none, .nvals = 0u));
    TEST_ASSERT_EQUAL_STRING("", out);
}
...
void test_append_builds_on_what_is_there(void)
{
    ...
    TEST_ASSERT_EQUAL_size_t(4u, EMBED_CALL(numer.build, NumerosCfg, .out = out, .cap = sizeof out, .spec = head,
                                            .vals = s_none, .nvals = 0u));
    TEST_ASSERT_EQUAL_size_t(9u, EMBED_CALL(numer.append, NumerosCfg, .out = out, .cap = sizeof out, .spec = tail,
                                            .vals = s_none, .nvals = 0u));
...
void test_append_guards_its_arguments(void)
{
    ...
    TEST_ASSERT_EQUAL_size_t(0u, EMBED_CALL(numer.append, NumerosCfg, .out = out, .cap = (size_t)0, .spec = spec,
                                            .vals = s_none, .nvals = 0u));
}
...
void test_append_that_does_not_fit_leaves_the_original(void)
{
    ...
    TEST_ASSERT_EQUAL_size_t(0u, EMBED_CALL(numer.append, NumerosCfg, .out = out, .cap = sizeof out, .spec = spec,
                                            .vals = s_none, .nvals = 0u));
...
void test_append_to_a_full_buffer(void)
{
    ...
    TEST_ASSERT_EQUAL_size_t(
        0u, EMBED_CALL(numer.append, NumerosCfg, .out = out, .cap = 3u, .spec = spec, .vals = s_none, .nvals = 0u));
}
...
void test_emit_guards_its_arguments(void)
{
    char out[32];
    TEST_ASSERT_EQUAL_size_t(
        0u, EMBED_CALL(numer.emit, NumerosCfg, .out = out, .cap = (size_t)0, .vals = s_none, .nvals = 0u));
}
...
void test_emit_append_guards_its_arguments(void)
{
    char out[32] = "x";
    TEST_ASSERT_EQUAL_size_t(
        0u, EMBED_CALL(numer.emit_append, NumerosCfg, .out = out, .cap = (size_t)0, .vals = s_none, .nvals = 0u));
}
...
void test_emit_covers_the_width_bearing_kinds(void)
{
    ...
    TEST_ASSERT_GREATER_THAN_size_t(
        0u, EMBED_CALL(numer.emit, NumerosCfg, .out = out, .cap = sizeof out, .vals = fields, .nvals = 4u));
    TEST_ASSERT_TRUE(EMBED_CALL(cellul.has, CatenaFinitaCfg, .src = out, .cap = sizeof out, .other = "010",
                                .other_cap = 4u, .ci = EMBED_FALSE));
    TEST_ASSERT_TRUE(EMBED_CALL(cellul.has, CatenaFinitaCfg, .src = out, .cap = sizeof out, .other = "0007",
                                .other_cap = 5u, .ci = EMBED_FALSE));
}
...
void test_a_g_field_with_no_width_gets_six_digits(void)
{
    ...
    TEST_ASSERT_GREATER_THAN_size_t(0u, EMBED_CALL(numer.build, NumerosCfg, .out = spec_out, .cap = sizeof spec_out,
                                                   .spec = spec, .vals = v, .nvals = 1u));
    TEST_ASSERT_GREATER_THAN_size_t(
        0u, EMBED_CALL(numer.emit, NumerosCfg, .out = emit_out, .cap = sizeof emit_out, .vals = v, .nvals = 1u));

    TEST_ASSERT_EQUAL_STRING_MESSAGE(spec_out, emit_out, "the spec path and the variadic path disagree on the default");
    TEST_ASSERT_EQUAL_size_t_MESSAGE(8u,
                                     EMBED_CALL(cellul.len, CatenaFinitaCfg, .src = spec_out, .cap = sizeof spec_out),
...
void test_a_hex_field_with_no_width_gets_one_digit(void)
{
    ...
    TEST_ASSERT_EQUAL_size_t(
        1u, EMBED_CALL(numer.build, NumerosCfg, .out = out, .cap = sizeof out, .spec = spec, .vals = v, .nvals = 1u));
...
void test_an_oct_field_with_no_width_gets_one_digit(void)
{
    ...
    TEST_ASSERT_EQUAL_size_t(
        1u, EMBED_CALL(numer.emit, NumerosCfg, .out = out, .cap = sizeof out, .vals = fields, .nvals = 1u));
    TEST_ASSERT_EQUAL_STRING("0", out);
}
...
void test_append_to_a_buffer_with_no_terminator_is_refused(void)
{
    ...
    TEST_ASSERT_EQUAL_size_t_MESSAGE(
        0u, EMBED_CALL(numer.emit_append, NumerosCfg, .out = out, .cap = sizeof out, .vals = s_none, .nvals = 0u),
...
void test_the_emit_path_carries_every_kind(void)
{
    ...
    TEST_ASSERT_GREATER_THAN_size_t(0u, EMBED_CALL(numer.emit, NumerosCfg, .out = out, .cap = sizeof out,
                                                   .vals = fields, .nvals = sizeof fields / sizeof fields[0]));

    TEST_ASSERT_TRUE_MESSAGE(EMBED_CALL(cellul.has, CatenaFinitaCfg, .src = out, .cap = sizeof out, .other = "2",
                                        .other_cap = 2u, .ci = EMBED_FALSE),
                             "the 64 bit value is missing");
    TEST_ASSERT_TRUE(EMBED_CALL(cellul.has, CatenaFinitaCfg, .src = out, .cap = sizeof out, .other = "-3",
                                .other_cap = 3u, .ci = EMBED_FALSE));
    TEST_ASSERT_TRUE(EMBED_CALL(cellul.has, CatenaFinitaCfg, .src = out, .cap = sizeof out, .other = "\"j\"",
                                .other_cap = 4u, .ci = EMBED_FALSE));
    TEST_ASSERT_TRUE(EMBED_CALL(cellul.has, CatenaFinitaCfg, .src = out, .cap = sizeof out, .other = "&lt;x&gt;",
...
void test_a_u64_of_its_largest_value(void)
{
    ...
    TEST_ASSERT_EQUAL_size_t(
        20u, EMBED_CALL(numer.emit, NumerosCfg, .out = out, .cap = sizeof out, .vals = fields, .nvals = 1u));
...
void test_an_emit_append_that_does_not_fit_puts_the_terminator_back(void)
{
    ...
    TEST_ASSERT_EQUAL_size_t_MESSAGE(
        0u, EMBED_CALL(numer.emit_append, NumerosCfg, .out = out, .cap = sizeof out, .vals = fields, .nvals = 1u),
