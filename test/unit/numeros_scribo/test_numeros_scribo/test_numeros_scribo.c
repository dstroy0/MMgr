...
void test_emit_rejects_an_unknown_kind(void)
{
    ...
    TEST_ASSERT_EQUAL_size_t(0u,
                             MMGR_CALL(numer.emit, NumerosCfg, .out = b, .cap = sizeof b, .vals = &bad, .nvals = 1u));
...
void test_emit_of_nothing_is_empty_not_garbage(void)
{
    ...
    b[0] = 'x';
    TEST_ASSERT_EQUAL_size_t(0u,
                             MMGR_CALL(numer.emit, NumerosCfg, .out = b, .cap = sizeof b, .vals = s_none, .nvals = 0u));
    TEST_ASSERT_EQUAL_STRING("", b);
}
...
void test_build_renders_a_spec(void)
{
    ...
    TEST_ASSERT_EQUAL_size_t(
        5u, MMGR_CALL(numer.build, NumerosCfg, .out = out, .cap = sizeof out, .spec = spec, .vals = v, .nvals = 1u));
...
void test_build_rejects_a_value_of_the_wrong_kind(void)
{
    ...
    TEST_ASSERT_EQUAL_size_t_MESSAGE(
        0u, MMGR_CALL(numer.build, NumerosCfg, .out = out, .cap = sizeof out, .spec = spec, .vals = v, .nvals = 1u),
...
void test_build_rejects_too_few_and_too_many_values(void)
{
    ...
    TEST_ASSERT_EQUAL_size_t(
        0u, MMGR_CALL(numer.build, NumerosCfg, .out = out, .cap = sizeof out, .spec = spec, .vals = one, .nvals = 1u));
    TEST_ASSERT_EQUAL_size_t_MESSAGE(
        0u, MMGR_CALL(numer.build, NumerosCfg, .out = out, .cap = sizeof out, .spec = spec, .vals = three, .nvals = 3u),
...
void test_build_guards_its_arguments(void)
{
    ...
    TEST_ASSERT_EQUAL_size_t(
        0u, MMGR_CALL(numer.build, NumerosCfg, .out = out, .cap = (size_t)0, .spec = spec, .vals = v, .nvals = 1u));
    TEST_ASSERT_EQUAL_size_t_MESSAGE(
        0u, MMGR_CALL(numer.build, NumerosCfg, .out = out, .cap = sizeof out, .spec = spec, .vals = v, .nvals = 0u),
...
void test_build_of_a_spec_with_only_literals(void)
{
    ...
    TEST_ASSERT_EQUAL_size_t(6u, MMGR_CALL(numer.build, NumerosCfg, .out = out, .cap = sizeof out, .spec = spec,
...
void test_build_covers_every_kind(void)
{
    ...
    TEST_ASSERT_GREATER_THAN_size_t(
        0u, MMGR_CALL(numer.build, NumerosCfg, .out = out, .cap = sizeof out, .spec = spec, .vals = v, .nvals = 7u));
    TEST_ASSERT_TRUE(MMGR_CALL(cellul.has, CatenaFinitaCfg, .src = out, .cap = sizeof out, .other = "\"j\"",
                               .other_cap = 4u, .ci = MMGR_FALSE));
    TEST_ASSERT_TRUE(MMGR_CALL(cellul.has, CatenaFinitaCfg, .src = out, .cap = sizeof out, .other = "&lt;x&gt;",
                               .other_cap = 10u, .ci = MMGR_FALSE));
}
...
void test_build_covers_the_width_bearing_kinds(void)
{
    ...
    TEST_ASSERT_GREATER_THAN_size_t(
        0u, MMGR_CALL(numer.build, NumerosCfg, .out = out, .cap = sizeof out, .spec = spec, .vals = v, .nvals = 5u));
    TEST_ASSERT_TRUE_MESSAGE(MMGR_CALL(cellul.has, CatenaFinitaCfg, .src = out, .cap = sizeof out, .other = "0007",
                                       .other_cap = 5u, .ci = MMGR_FALSE),
                             "DEC pads to its width");
    TEST_ASSERT_TRUE_MESSAGE(MMGR_CALL(cellul.has, CatenaFinitaCfg, .src = out, .cap = sizeof out, .other = "00ab",
                                       .other_cap = 5u, .ci = MMGR_FALSE),
                             "HEX pads to its width");
    TEST_ASSERT_TRUE_MESSAGE(MMGR_CALL(cellul.has, CatenaFinitaCfg, .src = out, .cap = sizeof out, .other = "10",
...
void test_build_overflow_leaves_an_empty_buffer(void)
{
    ...
    TEST_ASSERT_EQUAL_size_t(0u, MMGR_CALL(numer.build, NumerosCfg, .out = out, .cap = sizeof out, .spec = spec,
                                           .vals = s_none, .nvals = 0u));
    TEST_ASSERT_EQUAL_STRING("", out);
}
...
void test_append_builds_on_what_is_there(void)
{
    ...
    TEST_ASSERT_EQUAL_size_t(4u, MMGR_CALL(numer.build, NumerosCfg, .out = out, .cap = sizeof out, .spec = head,
                                           .vals = s_none, .nvals = 0u));
    TEST_ASSERT_EQUAL_size_t(9u, MMGR_CALL(numer.append, NumerosCfg, .out = out, .cap = sizeof out, .spec = tail,
                                           .vals = s_none, .nvals = 0u));
...
void test_append_guards_its_arguments(void)
{
    ...
    TEST_ASSERT_EQUAL_size_t(0u, MMGR_CALL(numer.append, NumerosCfg, .out = out, .cap = (size_t)0, .spec = spec,
                                           .vals = s_none, .nvals = 0u));
}
...
void test_append_that_does_not_fit_leaves_the_original(void)
{
    ...
    TEST_ASSERT_EQUAL_size_t(0u, MMGR_CALL(numer.append, NumerosCfg, .out = out, .cap = sizeof out, .spec = spec,
                                           .vals = s_none, .nvals = 0u));
...
void test_append_to_a_full_buffer(void)
{
    ...
    TEST_ASSERT_EQUAL_size_t(
        0u, MMGR_CALL(numer.append, NumerosCfg, .out = out, .cap = 3u, .spec = spec, .vals = s_none, .nvals = 0u));
}
...
void test_emit_guards_its_arguments(void)
{
    char out[32];
    TEST_ASSERT_EQUAL_size_t(
        0u, MMGR_CALL(numer.emit, NumerosCfg, .out = out, .cap = (size_t)0, .vals = s_none, .nvals = 0u));
}
...
void test_emit_append_guards_its_arguments(void)
{
    char out[32] = "x";
    TEST_ASSERT_EQUAL_size_t(
        0u, MMGR_CALL(numer.emit_append, NumerosCfg, .out = out, .cap = (size_t)0, .vals = s_none, .nvals = 0u));
}
...
void test_emit_covers_the_width_bearing_kinds(void)
{
    ...
    TEST_ASSERT_TRUE(MMGR_CALL(cellul.has, CatenaFinitaCfg, .src = out, .cap = sizeof out, .other = "010",
                               .other_cap = 4u, .ci = MMGR_FALSE));
    TEST_ASSERT_TRUE(MMGR_CALL(cellul.has, CatenaFinitaCfg, .src = out, .cap = sizeof out, .other = "0007",
                               .other_cap = 5u, .ci = MMGR_FALSE));
}
...
void test_a_g_field_with_no_width_gets_six_digits(void)
{
    ...
    TEST_ASSERT_GREATER_THAN_size_t(0u, MMGR_CALL(numer.build, NumerosCfg, .out = spec_out, .cap = sizeof spec_out,
    ...
    TEST_ASSERT_EQUAL_size_t_MESSAGE(8u,
                                     MMGR_CALL(cellul.len, CatenaFinitaCfg, .src = spec_out, .cap = sizeof spec_out),
...
void test_a_hex_field_with_no_width_gets_one_digit(void)
{
    ...
    TEST_ASSERT_EQUAL_size_t(
        1u, MMGR_CALL(numer.build, NumerosCfg, .out = out, .cap = sizeof out, .spec = spec, .vals = v, .nvals = 1u));
...
void test_append_to_a_buffer_with_no_terminator_is_refused(void)
{
    ...
    TEST_ASSERT_EQUAL_size_t_MESSAGE(
        0u, MMGR_CALL(numer.emit_append, NumerosCfg, .out = out, .cap = sizeof out, .vals = s_none, .nvals = 0u),
...
void test_the_emit_path_carries_every_kind(void)
{
    ...
    TEST_ASSERT_GREATER_THAN_size_t(0u, MMGR_CALL(numer.emit, NumerosCfg, .out = out, .cap = sizeof out, .vals = fields,
                                                  .nvals = sizeof fields / sizeof fields[0]));

    TEST_ASSERT_TRUE_MESSAGE(MMGR_CALL(cellul.has, CatenaFinitaCfg, .src = out, .cap = sizeof out, .other = "2",
                                       .other_cap = 2u, .ci = MMGR_FALSE),
                             "the 64 bit value is missing");
    TEST_ASSERT_TRUE(MMGR_CALL(cellul.has, CatenaFinitaCfg, .src = out, .cap = sizeof out, .other = "-3",
                               .other_cap = 3u, .ci = MMGR_FALSE));
    TEST_ASSERT_TRUE(MMGR_CALL(cellul.has, CatenaFinitaCfg, .src = out, .cap = sizeof out, .other = "\"j\"",
                               .other_cap = 4u, .ci = MMGR_FALSE));
    TEST_ASSERT_TRUE(MMGR_CALL(cellul.has, CatenaFinitaCfg, .src = out, .cap = sizeof out, .other = "&lt;x&gt;",
...
void test_an_emit_append_that_does_not_fit_puts_the_terminator_back(void)
{
    ...
    TEST_ASSERT_EQUAL_size_t_MESSAGE(
        0u, MMGR_CALL(numer.emit_append, NumerosCfg, .out = out, .cap = sizeof out, .vals = fields, .nvals = 1u),
