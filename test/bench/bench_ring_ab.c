...
static void report(const char *impl, const char *name, size_t bytes, double cycles)
{
    ...
    {
        printf("%s,%s,%u,%.3f,%.1f,%lu\n", impl, name, (unsigned)bytes, ops / 1e6,
               ((double)bytes * ops) / 1e6, g_refused);
    }
    fflush(stdout);
    g_refused = 0ul;
}
...
{
    while (MMGR_CALL(iteratio_infinita.put, InfinCfg, .ring = &g_mmgr_ring, .src = g_src, .bytes = 512u))
    {
    }
}
...
#define AB_PUT(N)                                                                                                      \
    ...
    {                                                                                                                  \
        ...
        BENCH_TIME_CYCLES(cy_, ITERS, {                                                                                \
            ...
            {                                                                                                          \
                const size_t h_ = protocore_ring_write_span(g_proto_buf, CAP,                                          \
                                                            PROTO_ATOMIC_LOAD(&g_proto_head), g_src, (N));             \
        ...
        g_refused = 0ul;                                                                                               \
        BENCH_TIME_CYCLES(cy_, ITERS, {                                                                                \
            if (!MMGR_CALL(iteratio_infinita.put, InfinCfg, .ring = &g_mmgr_ring, .src = g_src, .bytes = (N)))         \
            {                                                                                                          \
                g_refused++;                                                                                           \
            }                                                                                                          \
            MMGR_CALL(iteratio_infinita.consume, InfinCfg, .ring = &g_mmgr_ring, .bytes = (N));                        \
...
#define AB_PEEK(N)                                                                                                     \
    ...
    {                                                                                                                  \
        ...
        mmgr_fill();                                                                                                   \
        BENCH_TIME_CYCLES(cy_, ITERS, {                                                                                \
            if (MMGR_CALL(iteratio_infinita.read, InfinCfg, .ring = &g_mmgr_ring, .dst = g_dst, .bytes = (N)) == 0u)   \
            {                                                                                                          \
                mmgr_reset();                                                                                          \
                mmgr_fill();                                                                                           \
            }                                                                                                          \
...
int main(void)
{
...

    {
        ...
        BENCH_TIME_CYCLES(cy, ITERS, {
            BENCH_KEEP(protocore_ring_available(&g_proto_head, &g_proto_tail, CAP));
        });
        report("protocore", "available", 0u, cy);

        BENCH_TIME_CYCLES(cy, ITERS, {
            BENCH_KEEP(MMGR_CALL(iteratio_infinita.available, InfinCfg, .ring = &g_mmgr_ring));
        });
...

    {
        ...
        mmgr_reset();
        (void)MMGR_CALL(iteratio_infinita.put, InfinCfg, .ring = &g_mmgr_ring, .src = g_src, .bytes = 512u);
        BENCH_TIME_CYCLES(cy, ITERS, {
            if (!MMGR_CALL(iteratio_infinita.read_byte, InfinCfg, .ring = &g_mmgr_ring, .dst = &b))
            {
                mmgr_reset();
                (void)MMGR_CALL(iteratio_infinita.put, InfinCfg, .ring = &g_mmgr_ring, .src = g_src, .bytes = 512u);
            }
