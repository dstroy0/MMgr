 ...
 *
 * @note Fields match ExternumCfg, without the const on its six figures and the one on its pp pointer.
 * @note exter_place reads the six figures; the four pingpong backends read pp alone. MMGR_CALL zeroes
 *       the members an entry is not given.
 */
typedef struct
{
 ...
 * @note The store below is what puts fill_idx in range; the other three backends read it as it stands.
 * @warning args->pp must not be null. Nothing checks it and no assertion covers it, and the store
 *          below goes through it at once.
 */
 ...
 * @return      0 or 1.
 * @warning args->pp must not be null. Nothing checks it and no assertion covers it.
 * @warning Hands back fill_idx as it stands. It is 0 or 1 on a pair exter_pingpong_init has set, and
 *          whatever the storage held on one that has not been.
 */
 ...
 * @note Flips the low bit of fill_idx rather than keeping a second member, so the two indexes cannot
 *       drift apart.
 * @warning args->pp must not be null. Nothing checks it and no assertion covers it.
 * @warning The answer is an index only while fill_idx is 0 or 1. On a pair exter_pingpong_init has not
 *          set it is the flip of whatever the storage held.
 */
 ...
 * @note The four pingpong lines pass args->pp through as it stands [BORROWS]. MMGR_CALL builds its
 *       literal inside the emitted function, so the literal lives for that call alone and the pair has
 *       to outlive it. Nothing here copies the pair or frees it.
 * @warning No line tests what it forwards. A null pp reaches a backend that dereferences it with no
 *          check and no assertion.
 */
