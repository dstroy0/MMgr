...
int mmgr_memor_cmp(const void *a, const void *b, size_t n)
{
    ...
    {
        mmgr_scrut_word d = scrut.load(x + i) ^ scrut.load(y + i);
        if (d != 0)
        {

            i += scrut.zero_lane(MMGR_VERBUM_SCRUTOR_HIGH & ~scrut.has_zero(d));
...
const void *mmgr_memor_chr(const void *p, size_t n, uint8_t c)
{
    ...
    while (i + MMGR_SWAR_BYTES <= n)
    {
        mmgr_scrut_word m = scrut.eq(scrut.load_al(s + i), c, MMGR_FALSE);
        if (m != 0)
        {
            return s + i + scrut.zero_lane(m);
        }
        i += MMGR_SWAR_BYTES;
    }
