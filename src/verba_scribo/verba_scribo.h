 ...
 * @return         MMGR_TRUE when the sign bit is set, with nothing written.
 ...
 *
 * @param[in] args The value to test, as args->real [BORROWS].
 * @return         MMGR_TRUE for either infinity, with nothing written.
 * @note Wants the exponent field all ones and the mantissa zero, where is_nan wants it non-zero.
 * @note The sign takes no part, so a negative infinity returns MMGR_TRUE too.
 */
 ...
 *
 * @param[in] args The value to test, as args->real [BORROWS].
 * @return         MMGR_TRUE for any NaN, with nothing written.
