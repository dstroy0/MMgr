 ...
 * @warning out must be writable for cap bytes; a byte is always held back for the terminator finish stores.
 */
typedef struct
{
 ...
 *
 * @note MMGR_NS_LAYOUT asserts the ch member is at offset 0 and that the struct holds nothing else.
 */
typedef struct
{
 ...
 *
 * @note MMGR_NS_LAYOUT asserts the seven members sit at consecutive MMGR_FP_SIZE offsets, with nothing else.
 */
typedef struct
{
    ...
    size_t (*u64)(const VerbaNumerusCfg *args);      /**< The same walk as u32; the name says the width meant. */
 ...
 * @warning args->out must be writable for args->cap bytes; a byte is always held back for the terminator finish stores.
 */
 ...
 * @note The fraction rounds with ties to even, and one that carries raises the integer part.
 ...
 * @return      MMGR_TRUE when the sign bit is set; nothing is written.
 ...
 *
 * @param[in] args The value to test, as args->real [BORROWS].
 * @return      MMGR_TRUE for either infinity; nothing is written.
 ...
 *
 * @param[in] args The value to test, as args->real [BORROWS].
 * @return      MMGR_TRUE for any NaN; nothing is written.
...
/**
 * @brief Dispatch table instance named verba_textus; each member calls the matching mmgr_verba_ function.
 */
...
/**
 * @brief Dispatch table instance named verba_littera; its one member calls mmgr_verba_ch.
 */
...
/**
 * @brief Dispatch table instance named verba_numerus; each member calls the matching mmgr_verba_ function.
 */
...
/**
 * @brief Dispatch table instance named verba_fractio; each member calls the matching mmgr_verba_ function.
 */
...
/**
 * @brief Dispatch table instance named verba_finis; each member calls the matching mmgr_verba_ function.
 */
