...
{
    return mmgr_arena_scratch_alloc_aligned(a, n, MMGR_ARENA_ALIGN);
}
...
mmgr_bool mmgr_arena_set_add(mmgr_arena_set *s, void *base, size_t size)
{
    ...
    mmgr_arena *r = &s->region[s->count];
    mmgr_arena_init(r, base, size);
...
void *mmgr_arena_set_persist_alloc(mmgr_arena_set *s, size_t n)
{
    ...
    {
        void *p = mmgr_arena_persist_alloc(&s->region[i], n);
        if (p)
        {
            return p;
        }
...
void mmgr_arena_set_persist_free(mmgr_arena_set *s, void *p)
{
    ...
    for (size_t i = 0; i < s->count; i++)
    {
        mmgr_arena *r = &s->region[i];
        if (b >= r->base && b < r->base + r->size)
        {
            mmgr_arena_persist_free(r, p);
            return;
        }
    }
}
...
void *mmgr_arena_set_scratch_alloc_aligned(mmgr_arena_set *s, size_t n, size_t align)
{
    for (size_t i = 0; i < s->count; i++)
    {
        void *p = mmgr_arena_scratch_alloc_aligned(&s->region[i], n, align);
        if (p)
        {
            return p;
        }
...
{
    return mmgr_arena_set_scratch_alloc_aligned(s, n, MMGR_ARENA_ALIGN);
}
...
void mmgr_arena_set_scratch_release(mmgr_arena_set *s, const mmgr_arena_mark *m)
{
    ...
    {
        mmgr_arena_scratch_release(&s->region[i], m->top[i]);
    }
}
...
void mmgr_arena_set_scratch_reset(mmgr_arena_set *s)
{
    for (size_t i = 0; i < s->count; i++)
    {
        mmgr_arena_scratch_reset(&s->region[i]);
    }
}
...
size_t mmgr_arena_set_free_bytes(const mmgr_arena_set *s)
{
    ...
    for (size_t i = 0; i < s->count; i++)
    {
        t += mmgr_arena_free_bytes(&s->region[i]);
    }
    return t;
}
...
size_t mmgr_arena_set_persist_used(const mmgr_arena_set *s)
{
    ...
    for (size_t i = 0; i < s->count; i++)
    {
        t += mmgr_arena_persist_used(&s->region[i]);
    }
    return t;
}
...
size_t mmgr_arena_set_scratch_used(const mmgr_arena_set *s)
{
    ...
    for (size_t i = 0; i < s->count; i++)
    {
        t += mmgr_arena_scratch_used(&s->region[i]);
    }
    return t;
}
