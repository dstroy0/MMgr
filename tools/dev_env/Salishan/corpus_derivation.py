...
# the fewest that can show a trend rather than a coincidence between two neighbours.
STEADY = 3
...
def anchor_resolution(pure):
    ...
    # own contarget, so a dialect that will not separate names itself here.
    separations = {}
    for name in sorted(profiles):
        ...
        for seed in range(SHUFFLES):
            ...
            random.Random(seed).shuffle(shuffled)
            mixed.append(self_distance(shuffled))
        separations[name] = (distance(profiles[name], contarget), statistics.median(mixed))
...
def corpus_at(readers):
    ...
    EVERY is in the order the papers were read, so accumulating along it is the history rather than
    a reconstruction of one. A reader point sits at the size the corpus had reached when that paper
    was added, which is what puts it on the same axis as the algorithm curve beside it.
    """
    sizes = {}
    for name, stem, record, repair, marks in EVERY:
...
def placeable(held, separations, growth):
    ...
    distance from the pooled contarget clears MARGIN times the resolution of the worse of the two.
    ...
    if not separations:
        return 0.0
    reach = 0
    for name, (apart, contarget) in separations.items():
        resolution = max(held[name][2], contarget) / math.sqrt(growth)
...
def measured():
    ...
    for name, stem, record, repair, marks in EVERY:
        ...
        if counts is None:
            continue
        table = os.path.join(oracle.HERE, name)
...
def main():
    ...
    # and pooling thirteen of them into one denominator would report a rate nothing is sampling.
    ...
    with open(TARGET, "w", encoding="utf-8", newline="\n") as handle:
        ...
        handle.write("**Why the left panel is here.** The bound in Section 3 is a single number "
                     ...
                     "nine. The panel puts every paper on the picture so the shape of the evidence "
        ...
        handle.write("**What to read off it.** Every paper is one point, trials against failures. "
                     "The nine on the zero line are the whole of the evidence. The four above it "
        ...
        handle.write("**What to read off it.** The algorithm arm is the fraction of dialects whose "
                     "distance from the rest of the corpus pooled as contarget clears twice their "
                     "resolution. It starts at zero, because a small corpus resolves nothing, and "
                     "it crosses the reader arm at %.3g byte pairs, which is behind the corpus "
                     "already on disk. Both arms are drawn: byte pairs alone, and byte pairs with "
                     "the dialect's own alphabet applied first. Comparing an algorithm given "
                     "nothing against a person given a page is not a fair comparison, and the "
                     "alphabet is the first part of what the extraction already knows that it can "
                     "be handed.\n\n" % (crossing or 0.0))
        ...
        handle.write("The reader counts are not a channel here. A reader is written for one paper, "
                     ...
                     "the next paper shares. Pooling thirteen of them into one denominator would "
        ...
        # The archive is 993 papers and nine sound ones are counted, so the lifetime is what those
        # nine scale to. The bound is 3/N in each channel and N grows with the papers read.
        ...
        handle.write("* **Direction one and direction two share a source.** Both ask questions of "
                     ...
                     "source are excluded from the count rather than given a worse bound.\n")
        ...
        handle.write("* **shape**, two forms of one group sharing a leading or trailing run of "
                     "four characters. Salish morphology is heavily affixed and reduplicating, so "
                     "a shared run is usually a shared root or affix. It is a measurement and not "
                     "a parse.\n")
        ...
        handle.write("Comparing whole distributions does not work at this size. The method "
                     ...
                     "that route would need: about 2.9 times the labelled Lushootseed now on "
                     "disk.\n\n")
        if standing:
            handle.write("Asking for one term at a time does work, and it is the same move the "
                         ...
                         "where the pooled total alone would put it, so a run carrying no border "
                         "information contributes nothing however common it is. Each run is then "
                         "its own test, and a run needs enough of itself rather than enough of the "
                         "language.\n\n")
