...
Thirteen of the eighteen verify against their paper in both directions: no form written that the paper does not hold, no token in the paper that no row covers. Four of the other five are the papers whose extracted text is not what the page says, and what their remaining disagreements measure is the damage in that text. The two sections after this one are about them. The fifth is Kim, whose four direction-one failures are the extraction doubling a combining mark on a phonetic form; transcribing the doubled form to close them would put a corrupt string in the table, so they stay open and are counted.
...
| Kim, ICSNL 52 | 362 | 295 | 67 not found, 120 invented, 0 in the wrong language; 18 the reader flagged instead of guessing at |
...
`corpus_derivation.py` turns what those two and `coverage_check.py` report into a bound on the per-line error rate, and rewrites `corpus-derivation.md` from it after every change to a table. It reads the checks by running them and parsing their own output rather than recomputing what they compute, because an earlier version reimplemented the same quantities and drifted: it reported 16 direction-one failures where `oracle_check` reports 0. The bound now stands at 3.06e-11 per line across three channels. Two of the three have seen a failure and are reported at their observed rate; the third is at the rule-of-three bound over 6496 trials. It moved from 1.85e-10 to 3.06e-11 over the papers added since, and it moved the wrong way once, from 6.03e-11 up to 6.83e-11, when a reader turned out to be losing forms. No digit of its mantissa has settled, and the document's section 4 names the three places its independence is thin. The reader counts are deliberately not one of the three channels: a reader is written for one paper, so its error rate is a fact about that paper and pooling twelve of them would report a rate nothing is sampling.
...
`build/papers/` holds 152 extracted paper texts and 146 of the PDFs they came from. Seventeen have a reader and a hand extraction and one more has a hand extraction alone, so 134 of the 152 are unread and 841 of the archive's 993 are not fetched. Six of the 146 are in the class the section above describes, and their texts are the font's encoding.
