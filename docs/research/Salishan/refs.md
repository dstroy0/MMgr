...
`corpus_derivation.py` turns what those two and `coverage_check.py` report into a bound on the per-line error rate, and rewrites `corpus-derivation.md` from it after every change to a table. It reads the checks by running them and parsing their own output rather than recomputing what they compute, because an earlier version reimplemented the same quantities and drifted: it reported 16 direction-one failures where `oracle_check` reports 0. The bound now stands at 1.85e-10 per line across three channels, none of which has seen a failure. That number is an upper bound from the rule of three and not a measurement, no digit of its mantissa has settled, and the document's section 4 names the three places its independence is thin. The reader counts are deliberately not one of the three channels: a reader is written for one paper, so its error rate is a fact about that paper and pooling twelve of them would report a rate nothing is sampling.
