...
`corpus_derivation.py` turns what those two and `coverage_check.py` report into a bound on the per-line error rate, and rewrites `corpus-derivation.md` from it after every change to a table. It reads the checks by running them and parsing their own output, and never recomputes what they compute, because an earlier version reimplemented the same quantities and drifted: it reported 16 direction-one failures where `oracle_check` reports 0. The bound now stands at 4.01e-11 per line across three channels. Two of the three have seen a failure and are reported at their observed rate; the third is at the rule-of-three bound over 6865 trials. It moved from 1.85e-10 to 3.06e-11 over the papers added since, and it has moved the wrong way twice: from 6.03e-11 up to 6.83e-11 when a reader turned out to be losing forms, and from 3.06e-11 up to 4.01e-11 when the Hall paper arrived carrying a token its record cannot give back. Both rises are the measurement working. A bound that only ever falls is measuring the papers that were easy to add. No digit of its mantissa has settled, and the document's section 4 names the three places its independence is thin. The reader counts are deliberately not one of the three channels: a reader is written for one paper, so its error rate is a fact about that paper and pooling twelve of them would report a rate nothing is sampling.
