...
```c
uint8_t buf[64];
mmgr_span w = EMBED_CALL(spat.from, SpatiumCfg, .buf = buf, .cap = sizeof buf);

EMBED_CALL(byteio.raw, OctetusCfg, .w = &w, .src = head, .bytes = sizeof head);
EMBED_CALL(byteio.put_be, OctetusCfg, .w = &w, .val = id, .bytes = 4u);
EMBED_CALL(byteio.raw, OctetusCfg, .w = &w, .src = body, .bytes = n);
...
and `read`.** One member per job, not one per entry.

**`after` and `first` return a new span; they do not move the one you passed.** Assign the result.

**A `.count` of exactly `cap` is not a failure.** It gives an empty span that has not failed: nothing
is left, and nothing went wrong either. Past `cap` is the failure.
...
span over temporary-tier storage from a function that rewinds its own mark is a use-after-free, and
no field in the span can tell you.
